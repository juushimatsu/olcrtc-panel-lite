...
![olcRTC Panel Lite repository cover](./cover.png)
...
При первом входе после установки панель предлагает wizard автоматической настройки. Он проверяет компоненты Playwright/Chromium/noVNC, помогает войти в WB Stream и Telemost и создаёт выбранные инстансы. Вход и CAPTCHA выполняются вручную в авторизованном окне noVNC; Room ID можно указать вручную, если автоматизация недоступна.

Wizard можно пропустить и настроить инстансы обычным способом. Состояние сохраняется в базе данных, поэтому после перезапуска панели доступно продолжение незавершённого шага. Повторный запуск доступен в `Настройки → Автоматизация WB и Telemost → Запустить wizard автонастройки`.

Подробное описание шагов и диагностики находится в [документации auto-setup](docs/auto-setup.md).
