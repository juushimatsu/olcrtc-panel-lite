...
function renderSubscriptions() {
  document.querySelector('#page-content').innerHTML = `
    <section class="page"><div class="page-header"><div class="page-title"><h1>Подписки</h1><p>Подписки содержат только URI, совместимые с OLCRTC Client</p></div><div class="header-actions"><button class="btn" data-action="import-subscriptions">⇧ Импорт</button><button class="btn" data-action="export-subscriptions">⇩ Экспорт</button><button class="btn btn-primary" data-action="create-subscription">＋ Подписка</button></div></div><div class="notice" style="margin-bottom:16px">OLCBOX подписки не поддерживает. Для OLCBOX используйте отдельный QR конкретного инстанса.</div>
...
function openEntryForm(slug) {
  const compatible = state.instances.filter(item => !clientQRUnavailable(item));
  openModal('Добавить entry OLCRTC Client', `<form data-form="entry" data-slug="${attr(slug)}"><div class="field"><label>Источник</label><select class="select" name="source_type" data-role="entry-source"><option value="linked">Linked instance</option><option value="manual">Manual URI</option></select></div><div data-entry-linked style="margin-top:15px" class="field"><label>Инстанс</label><select class="select" name="source_instance_id" ${compatible.length ? '' : 'disabled'}>${compatible.map(i=>`<option value="${i.id}">#${i.id} ${esc(i.name)}</option>`).join('')}</select>${compatible.length ? '' : '<span class="field-hint">Нет инстансов с поддерживаемой комбинацией и необходимым WB token.</span>'}</div><div data-entry-manual class="field hidden" style="margin-top:15px"><label>OLCRTC Client olcrtc:// URI</label><textarea class="textarea mono" name="raw_uri" placeholder="olcrtc://provider@r/room?k=...&t=...&c=..."></textarea><span class="field-hint">Принимаются только URI, которые разбирает OLCRTC Client. URI OLCBOX здесь не поддерживаются.</span></div><div class="form-grid" style="margin-top:15px">${field('name','Отображаемое имя','')}${field('comment','Комментарий','')}${field('ip','Показываемый IP','')}${field('color','Цвет','#0EA58C','color')}<label class="checkbox"><input type="checkbox" name="enabled" checked> Публиковать entry</label></div><div class="form-actions"><button class="btn" type="button" data-action="close-modal">Отмена</button><button class="btn btn-primary" type="submit">Добавить</button></div></form>`);
}
...
app.addEventListener('click', async event => {
  ...
  try {
    ...
    if (action === 'subscription-qr') await showSubscriptionQR(target.dataset.slug);
...
async function showSubscriptionQR(slug){
  const sub=state.subscriptions.find(item=>item.slug===slug);
  let warning='';
  if(sub?.mirror_enabled){
  ...
  const result=await api(`/api/v1/subscriptions/${encodeURIComponent(slug)}/payload`);
  const src=`/api/v1/subscriptions/${encodeURIComponent(slug)}/qr?format=client`;
  openQRPayloadModal('QR подписки OLCRTC Client',src,result.payload,maskSubscriptionBundle,warning);
}
