...
function instanceExpanded(item) {
  return `<tr class="expanded-row"><td colspan="11"><div class="expanded-content"><div class="expanded-actions">
    ...
    <button class="btn btn-small" data-action="edit-instance" data-id="${item.id}">✎ Изменить</button><button class="btn btn-small" data-action="instance-duplicate" data-id="${item.id}">⧉ Дублировать</button><button class="btn btn-small" data-action="instance-uri" data-id="${item.id}" data-format="standard">URI</button><button class="btn btn-small" data-action="instance-qr" data-id="${item.id}" data-format="standard">QR</button><button class="btn btn-small" data-action="instance-qr" data-id="${item.id}" data-format="exclave">Exclave QR</button><button class="btn btn-small" data-action="instance-rotate-key" data-id="${item.id}">⌘ Ротация key</button><button class="btn btn-small" data-action="instance-change-room" data-id="${item.id}">⌂ Room ID</button><button class="btn btn-small" data-action="instance-diagnostics" data-id="${item.id}">◇ Диагностика</button><button class="btn btn-small" data-action="instance-logs" data-id="${item.id}">⌁ Логи</button><button class="btn btn-small" data-action="instance-reset-traffic" data-id="${item.id}">↺ Сбросить трафик</button><button class="btn btn-danger btn-small" data-action="instance-delete" data-id="${item.id}">Удалить</button>
  </div><div class="expanded-stats">${detail('Upload', formatBytes(item.upload_bytes))}${detail('Download', formatBytes(item.download_bytes))}${detail('Последний трафик', item.last_traffic_at ? formatDate(item.last_traffic_at) : 'Нет данных')}${detail('Reset policy', item.reset_policy)}${detail('DNS', item.dns)}${detail('Совместимость', compatibility(item.provider,item.transport) || 'Стабильная комбинация')}</div></div></td></tr>`;
}
  ...
  openModal(item ? 'Изменить инстанс' : 'Новый инстанс', `
    <form data-form="instance" data-id="${i.id || ''}">
      <div class="form-grid">
        ...
        ${field('auth_token','WB account token','','password',item ? 'Оставьте пустым, чтобы не менять' : 'Только для WB, не входит в URI')}
      ...
      <div class="form-actions" style="justify-content:flex-start"><button class="btn" type="button" data-action="wb-fill-instance">Получить WB token и создать комнату</button></div>
...
function renderSubscriptions() {
  document.querySelector('#page-content').innerHTML = `
    <section class="page"><div class="page-header"><div class="page-title"><h1>Подписки</h1><p>Одна модель данных для standard, Exclave и encrypted mirror</p></div><div class="header-actions"><button class="btn" data-action="import-subscriptions">⇧ Импорт</button><button class="btn" data-action="export-subscriptions">⇩ Экспорт</button><button class="btn btn-primary" data-action="create-subscription">＋ Подписка</button></div></div>
...
app.addEventListener('click', async event => {
  ...
  try {
    ...
    if (action === 'delete-subscription') { if (confirm(`Удалить подписку ${target.dataset.slug}?`)) { await api(`/api/v1/subscriptions/${target.dataset.slug}`,{method:'DELETE'}); await loadSubscriptions(); toast('Подписка удалена'); } }
    if (action === 'subscription-qr') showSubscriptionQR(target.dataset.slug,target.dataset.format);
...
app.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (form.dataset.form === 'wb-token') { const d=new FormData(form);const result=await api('/api/v1/wb/token/refresh',{method:'POST',body:JSON.stringify({token:d.get('token')})});closeModal();toast('WB token обновлён',`Обновлено инстансов: ${result.updated?.length || 0}`); }
...
async function handleInstanceAction(action, target) {
  ...
  if (['start','stop','restart','duplicate','rotate-key','reset-traffic','diagnostics'].includes(simple)) {
  ...
  if (simple==='uri') { const format=target.dataset.format||'standard';const result=await api(`/api/v1/instances/${id}/uri?format=${format}`);openModal(`${format} URI`,`<div class="payload-box">${esc(result.uri)}</div><div class="form-actions"><button class="btn btn-primary" data-action="copy" data-value="${attr(result.uri)}">Копировать</button></div>`); }
  if (simple==='qr') showInstanceQR(id,target.dataset.format||'standard');
  if (simple==='logs') { state.logsUnit=`instance:${id}`;await navigate('logs'); }
}

function showInstanceQR(id,format){const src=`/api/v1/instances/${id}/qr?format=${format}`;openModal(`${format==='exclave'?'Exclave':'Стандартный'} QR`,`<div class="qr-wrap"><img src="${src}" alt="QR-код инстанса"><a class="btn" href="${src}" download>Скачать PNG</a></div>`);}
function showSubscriptionQR(slug,format){const src=`/api/v1/subscriptions/${encodeURIComponent(slug)}/qr?format=${format}`;openModal(`${format==='exclave'?'Exclave / Yandex':'Стандартный'} QR подписки`,`<div class="qr-wrap"><img src="${src}" alt="QR-код подписки"><a class="btn" href="${src}" download>Скачать PNG</a></div>`);}
...
async function fillWBInstanceForm(){
  ...
  const session=await api('/api/v1/wb/session',{method:'POST'});window.open(session.novnc_url,'olcrtc-wb-novnc','noopener');toast('WB-сессия запущена','Войдите в WB Stream и пройдите CAPTCHA.');
  for(let attempt=0;attempt<450;attempt++){
    await new Promise(resolve=>setTimeout(resolve,2000));
    const current=await api('/api/v1/wb/session');const phase=current.state?.phase;
    if(phase==='success'){const room=current.state?.room_id||'';if(room)form.elements.room_id.value=room;form.elements.provider.value='wbstream';toast('WB данные получены','Room ID заполнен, token сохранён на сервере.');return;}
    if(phase==='error')throw new Error(current.state?.message||'WB automation завершилась с ошибкой');
